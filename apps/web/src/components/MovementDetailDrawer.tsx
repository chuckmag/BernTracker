    ...
    <div>
      ...
      <ResponsiveContainer width="100%" height={140}>
        <LineChart data={data.points} margin={{ top: 4, right: 4, bottom: 0, left: 0 }}>
          ...
          <YAxis
            dataKey="value"
            tickFormatter={(v: number) => data.points[0]?.label ? String(v) : String(v)}
            tick={{ fontSize: 10, fill: textColor }}
            axisLine={false}
            tickLine={false}
            width={36}
...
export default function MovementDetailDrawer({ movementId, name, prTypes, onClose }: Props) {
  ...
  return (
    <>
      ...
      <div
        ...
        aria-modal="true"
        aria-label={name}
        className="fixed inset-0 md:inset-auto md:top-0 md:right-0 md:h-full md:w-96 md:border-l md:border-slate-200 dark:md:border-gray-800 bg-white dark:bg-gray-900 z-40 overflow-y-auto"
      >
        {/* Header */}
        <div className="sticky top-0 bg-white dark:bg-gray-900 border-b border-slate-200 dark:border-gray-800 px-4 py-3 flex items-center justify-between">
          <h2 className="font-semibold text-slate-950 dark:text-white truncate">{name}</h2>
          <button
            ref={closeRef}
            onClick={handleClose}
            aria-label="Close drawer"
            className="-mr-1 w-8 h-8 inline-flex items-center justify-center rounded text-slate-500 dark:text-gray-400 hover:text-slate-950 dark:hover:text-white focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-2 focus-visible:ring-offset-white dark:focus-visible:ring-offset-gray-900"
          >
            ✕
          </button>
        </div>

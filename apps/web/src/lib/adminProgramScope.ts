 ...
 * unaffiliated program. canSeeMembers is on so the Members tab is visible.
 * Invite flow is view-only (canManage=false in the tab) since there is no
 * gym roster to draw from. canSetDefault is off — gym default is a gym concept.
 */
import { api } from './api'

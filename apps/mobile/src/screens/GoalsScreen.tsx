...
function GoalListRow({ goal, onPress }: RowProps) {
  ...
  return (
    ...
    >
      <ThemedView variant="card" style={[s.row, { borderColor: colors.borderSubtle }]}>
        <GoalProgressRing
          percent={percent}
        ...
        <View style={s.rowText}>
          <ThemedText style={s.rowTitle} numberOfLines={1}>
            <ThemedText style={s.rowIcon}>{TYPE_ICON[goal.type]} </ThemedText>
            {goal.title}
          </ThemedText>
          <ThemedText variant="secondary" style={s.rowMeta} numberOfLines={1}>{formatProgressLabel(goal)}</ThemedText>
          <ThemedText variant="tertiary" style={s.rowSub} numberOfLines={1}>
...
export default function GoalsScreen() {
  ...
  return (
    <ThemedView variant="screen" style={s.container}>
      ...
      >
        <ThemedView variant="card" style={s.tabStrip} accessibilityRole="tablist">
          {(['ACTIVE', 'COMPLETED', 'ARCHIVED'] as GoalStatus[]).map((status) => (
            <TouchableOpacity
              key={status}
              style={[s.tab, tab === status && { backgroundColor: colors.borderSubtle }]}
        ...
        {!loading && error && (
          <ThemedView variant="card" style={s.errorBox}>
            <ThemedText style={[s.errorText, { color: colors.errorText }]}>{error}</ThemedText>
          </ThemedView>
        )}

        {!loading && !error && goals.length === 0 && (
          <ThemedView variant="card" style={s.empty}>
            <ThemedText style={s.emptyTitle}>No {TAB_LABELS[tab].toLowerCase()} goals</ThemedText>
      ...
      >
        <ThemedText style={[s.fabIcon, { color: colors.onPrimary }]}>+</ThemedText>
      </TouchableOpacity>
...
const s = StyleSheet.create({
  ...
  errorBox: { borderRadius: 12, padding: 16 },
  ...
  fabIcon: { fontSize: 26, lineHeight: 28, fontWeight: '700' },
})
